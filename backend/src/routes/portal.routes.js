...
router.get("/trades", async (req, res, next) => { try {
  const base = req.user.role === "MANAGER"
    ? db("trades").leftJoin("employee_clients as ownership", "trades.client_id", "ownership.client_id").leftJoin("employees as owner", "ownership.employee_id", "owner.id").select("trades.*", "owner.name as employee_name")
    : db("trades").select("trades.*");
...
router.post("/sync", requireManager, async (req, res) => { res.status(202).json({ message: "Sync started" }); runBseSync().then((result) => { if (result.success && req.app.get("io")) req.app.get("io").emit("data:updated", { syncRunId: result.syncRunId }); }); });
